...
| `05-upgrade.sh` | `05-upgrade.sh <base> [<branch>]` | Re-base all (or up to `<branch>`) feature branches: DAG roots onto `<base>`, children onto their parent's upgraded tip. Bridge plus cherry-pick. Auto-backup before and after; the post snapshot fires on every run that completes without a conflict (even a fully-skipped idempotent re-run) |
